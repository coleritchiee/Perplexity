...
public class EntityProj extends Projectile {
    ...
    public EntityProj(EntityType<? extends EntityProj> type, Level world, SpellResolver resolver){
        this(type, world, resolver.context.getCaster());
        this.resolver = resolver;
    }
    public EntityProj(Level world, SpellResolver resolver){
        this(Registration.SPELL_PROJ.get(), world, resolver);
    }

    public EntityProj(PlayMessages.SpawnEntity spawnEntity, Level level) {
        this(Registration.SPELL_PROJ.get(), level);
    }
    ...
    public void tick() {
        ...
        if((!level().isClientSide && this.age > 1200) || (!level().isClientSide && resolver == null)){
    ...
    protected void onHit(HitResult result) {
        ...
        if(!level().isClientSide && result instanceof EntityHitResult entityHitResult){
            if(this.resolver != null){
                this.resolver.processEffects(result);
                this.remove(RemovalReason.DISCARDED);
            }
        }
