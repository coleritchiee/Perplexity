...
public class NashorsToothEffect extends MobEffect {
    public NashorsToothEffect() {
        super(MobEffectCategory.NEUTRAL, 0xae16fa);
    }

    @SubscribeEvent
    public static void onAttack(LivingIncomingDamageEvent event) {
        if(event.getSource().getEntity() instanceof Player source){
            if(source.hasEffect(Registration.NASHORS_EFFECT)&&!source.equals(event.getEntity())) {
